...
models = {
    "light-inpainting": config.light_c64,
}
...
# read data
test_0 = torch.load(np.array(sorted(glob.glob(str(dataset_path / f"*_0_*"), recursive=True)))[0])
...
for model_idx, (name, model) in enumerate(models.items()):
    ...
    model.light3_3.dconv1.register_forward_hook(get_activation('dconv1'))
    model.light3_3.dconv2.register_forward_hook(get_activation('dconv2'))
    model.light3_3.dconv3.register_forward_hook(get_activation('dconv3'))
    model.light3_3.dconv4.register_forward_hook(get_activation('dconv4'))

    model.light3_3.uconv1.register_forward_hook(get_activation('uconv1'))
    model.light3_3.uconv2.register_forward_hook(get_activation('uconv2'))
    model.light3_3.uconv3.register_forward_hook(get_activation('uconv3'))

    model.light3_3.conv1.register_forward_hook(get_activation('conv1'))
    model.light3_3.conv2.register_forward_hook(get_activation('conv2'))
    ...
    cv.imwrite(str(folder_path / f"feature_map.png"), feature_map_img)
    cv.imwrite(str(folder_path / f"feature_map_normal.png"),
               cv.cvtColor(mu.visual_normal(gt, "GT", histogram=False), cv.COLOR_RGB2BGR))
    cv.imwrite(str(folder_path / f"feature_map_out.png"),
               cv.cvtColor(mu.visual_light(xout, "Out", histogram=False), cv.COLOR_RGB2BGR))
    cv.imwrite(str(folder_path / f"feature_map_vertex.png"),
