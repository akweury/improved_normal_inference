...
def parse_arguments():
    ...
    parser.add_argument('--desc', type=str, default='My training run for single-scale normal estimation.',
                        help='description')
    parser.add_argument('--indir', type=str, default='/home/sitzikbs/Datasets/pcpnet/',
                        help='input folder (point clouds)')
    parser.add_argument('--logdir', type=str, default=str(config.ws_path / "deepfit" / "log"),
                        help='training log folder')
    parser.add_argument('--trainset', type=str, default="synthetic_data_noise", help='training set file name')
    parser.add_argument('--train_on', type=str, default="full", help='choose training size')
    parser.add_argument('--testset', type=str, default='selval', help='test set file name')
    parser.add_argument('--saveinterval', type=int, default='10', help='save model each n epochs')
    parser.add_argument('--refine', action="store_true",
                        help='flag to refine the model, path determined by outri and model name')
    ...
    parser.add_argument('--points_per_patch', type=int, default=25, help='max. number of points per patch')
...
def train_pcpnet(opt):
    ...
    if (os.path.exists(log_dirname) or os.path.exists(
            model_filename)) and not opt.name == 'DeepFit_trainall' and opt.refine == '':
        if opt.overwrite:
            response = 'y'
        else:
    ...
    os.system('copy train_n_est.py %s' % (log_dirname))  # backup the current training file
    ...
    train_dataloader, test_dataloader = utils.create_dataloader(opt, eval_mode=False)

    # keep the exact training shape names for later reference
    opt.train_shapes = train_dataloader.dataset.__len__()
    opt.test_shapes = test_dataloader.dataset.__len__()
    ...
    if opt.scheduler_type == 'step':
        scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[200, 500, 700],
    ...
    train_num_batch = train_dataloader.batch_size
    test_num_batch = test_dataloader.batch_size
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, (data, target) in enumerate(train_dataloader):
            ...
            log_string('[%s %d: %d/%d] %s loss: %f' % (opt.name, epoch,
                                                       train_batchind, train_num_batch - 1, green('train'),
                                                       loss.item()), log_file)
            ...
            for test_batchind, (data, target) in enumerate(train_dataloader):
                model.eval()
        ...
        folder_path = os.path.join(out_dir, f"output_{chart.date_now}_{chart.time_now}")
        chart.line_chart(np.array([losses]), folder_path, log_y=True)
        ...
        if epoch % (
                5 * 10 ** math.floor(math.log10(max(2, epoch - 1)))) == 0 or epoch % 10 == 0 or epoch == opt.nepoch - 1:
        ...
        if o == 'unoriented_normals' or o == 'oriented_normals':
            ...
            if o == 'unoriented_normals':
                if normal_loss_type == 'ms_euclidean':
                    normal_loss = torch.min((o_pred - o_target).pow(2).sum(1),
                                            (o_pred + o_target).pow(2).sum(1)).mean() * output_loss_weight[oi]
            ...
            if use_consistency:
                ...
                if patch_rot is not None:
                    ...
                    o_pred = torch.bmm(o_pred.view(-1, 1, 3),
                                       patch_rot.transpose(2, 1).repeat(1, n_points, 1, 1).view(-1, 3, 3)).view(
                ...
                if normal_loss_type == 'ms_euclidean':
                    ...
                    consistency_loss = 0.25 * torch.mean(point_weights *
                                                         torch.norm(torch.cross(o_pred.view(-1, 3),
                                                                                o_target.view(-1, 3), dim=-1).view(
                                                             batch_size, -1, 3), p=2, dim=2))
                else:
                    raise ValueError('Unsupported loss type: %s' % (normal_loss_type))
...
def get_model(opt, log_dirname):
    ...
    os.system('copy %s %s' % (os.path.join(os.getcwd(), "DeepFit.py"), log_dirname))  # backup the current model
    return model
