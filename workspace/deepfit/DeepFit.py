...
from workspace.deepfit import normal_estimation_utils
...
def fit_Wjet(points, weights, order=2, compute_neighbor_normals=False):
    ...
    if order > 1:
        # pre conditioning
        h = (torch.mean(torch.abs(x), 1) + torch.mean(torch.abs(y),
                                                      1)) / 2  # absolute value added from https://github.com/CGAL/cgal/blob/b9e320659e41c255d82642d03739150779f19575/Jet_fitting_3/include/CGAL/Monge_via_jet_fitting.h
    ...
    if order == 1:
        ...
        h_3 = h_2 * h
        h_4 = h_3 * h
        D_inv = torch.diag_embed(
            1 / torch.cat([h, h, h_2, h_2, h_2, h_3, h_3, h_3, h_3, h_4, h_4, h_4, h_4, h_4, torch.ones_like(h)],
                          dim=1))
    else:
    ...
    n_est = torch.nn.functional.normalize(
        torch.cat([-beta[:, 0:2].squeeze(-1), torch.ones(batch_size, 1, device=x.device, dtype=beta.dtype)], dim=1),
        p=2, dim=1)

    if compute_neighbor_normals:
        beta_ = beta.squeeze(-1).unsqueeze(1).repeat(1, n_points, 1).unsqueeze(-1)
        if order == 1:
            ...
            neighbor_normals = torch.nn.functional.normalize(
                torch.cat([-(beta_[:, :, 0] + 2 * beta_[:, :, 2] * x + beta_[:, :, 4] * y + 3 * beta_[:, :, 5] * x_2 +
                             2 * beta_[:, :, 7] * xy + beta_[:, :, 8] * y_2 + 4 * beta_[:, :, 9] * x_3 + 3 * beta_[:, :,
                                                                                                             11] * x_2 * y
                             + beta_[:, :, 12] * y_3 + 2 * beta_[:, :, 13] * y_2 * x),
                           -(beta_[:, :, 1] + 2 * beta_[:, :, 3] * y + beta_[:, :, 4] * x + 3 * beta_[:, :, 6] * y_2 +
                             beta_[:, :, 7] * x_2 + 2 * beta_[:, :, 8] * xy + 4 * beta_[:, :, 10] * y_3 + beta_[:, :,
                                                                                                          11] * x_3 +
                             3 * beta_[:, :, 12] * x * y_2 + 2 * beta_[:, :, 13] * y * x_2),
                           torch.ones(batch_size, n_points, 1, device=x.device)], dim=2), p=2, dim=2)

    return beta.squeeze(-1), n_est, neighbor_normals
...
def solve_linear_system(XtX, XtY, sub_batch_size=None):
    ...
    for i in range(n_iterations):
        try:
            L = torch.linalg.cholesky(XtX[sub_batch_size * i:sub_batch_size * (i + 1), ...], upper=False)
            ...
            try:
                ...
                beta[sub_batch_size * i:sub_batch_size * (i + 1), ...], _ = \
                    torch.linalg.solve(XtY[XtX[sub_batch_size * i:sub_batch_size * (i + 1), ...],
                                sub_batch_size * i:sub_batch_size * (i + 1), ...])
    return beta


class PointNetFeatures(nn.Module):
    def __init__(self, num_points=500, num_scales=1, use_point_stn=False, use_feat_stn=False, point_tuple=1,
                 sym_op='max'):
...
def compute_principal_curvatures(beta):
    ...
    with torch.no_grad():
        if beta.shape[1] < 5:
            ...
            dirs = torch.cat([dirs, torch.zeros(dirs.shape[0], 2, 1, device=dirs.device)],
