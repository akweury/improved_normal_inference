...
def main():
    ...
    ############ NEW EXP MODE ############
        ...
        sys.path.append(config.exper_synthetic / "pncnn")

        print_args(args)
        exp_dir = str(config.exper_synthetic / 'pncnn')
        # Create dataloader
        train_loader, val_loader = create_dataloader(args, eval_mode=False)

        # import the model
        f = importlib.import_module("improved_normal_inference.pncnn.workspace.synthetic.pncnn.network")
