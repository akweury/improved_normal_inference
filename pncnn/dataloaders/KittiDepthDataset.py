...
class KittiDepthDataset(Dataset):
    ...
    def __init__(self, kitti_depth_path, setname='train', transform=None, norm_factor=256, invert_depth=False,
        ...
        self.kitti_depth_path = config.depth_path_kitti
        ...
        if setname in ['train', 'selval']:
            ...
            self.depth = np.array(sorted(
                glob.glob(str(self.kitti_depth_path / "proj_depth" / "velodyne_raw" / "image_02" / "*.png"),
                          recursive=True)))
            self.gt = np.array(sorted(
                glob.glob(str(config.gt_path_kitti / "proj_depth" / "groundtruth" / "image_02" / "*.png"),
                          recursive=True)))
            ...
            self.depth = np.array(sorted(
                glob.glob(str(config.val_sel_cropped_path / "velodyne_raw" / "*.png"), recursive=True)))
            self.gt = np.array(sorted(
                glob.glob(str(config.val_sel_cropped_path / "groundtruth_depth" / "*.png"), recursive=True)))
            ...
            self.depth = np.array(sorted(
                glob.glob(str(config.test_depth_path / "velodyne_raw" / "*.png"), recursive=True)))
            self.gt = np.array(sorted(
                glob.glob(str(config.test_depth_path / "velodyne_raw" / "*.png"), recursive=True)))
    ...
    def __getitem__(self, item):
        ...
        # Read RGB images 
        if self.load_rgb:
            if self.setname in ['train', 'selval']:
