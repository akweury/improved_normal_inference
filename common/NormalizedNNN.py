...
class NormalizedNNN(nn.Module):
    ...
    def forward(self, x0, c0):
        ...
        x1 = self.active(self.dconv1(x0 * c0))  # 512,512
        c1 = self.active(self.dconv1(c0))
        ...
        x2 = self.active(self.dconv2(x1_ds))  # 256,256
        ...
        x3 = self.active(self.dconv2(x2_ds))  # 128,128
        # c3 = self.dconv2(c2_ds)
        # x3 = x3 / (c3 + self.epsilon)
        # c3 = self.c_avg(c3, self.dconv2.weight)

        x3 = self.active(self.dconv3(x3))  # 128,128
        # c3 = self.dconv3(c3)
        # x3 = x3 / (c3 + self.epsilon)
        ...
        x5 = self.active(self.uconv1(torch.cat((x3, x4_us), 1)))  # 128, 128
        ...
        x6 = self.active(self.uconv2(torch.cat((x2, x5_us), 1)))  # 256, 256
        ...
        x7 = self.active(self.uconv3(torch.cat((x1, x6_us), 1)))
