...
class VILNet(nn.Module):
    def __init__(self, in_ch, out_ch, channel_num):
        ...
        self.merge = GConv(channel_size_2, channel_size_1, kernel_up, stride, padding_up)
        ...
        self.v_conv1 = GConv(channel_size_3, channel_size_1, kernel_up, stride, padding_up)
        self.conv1 = nn.Conv2d(channel_size_1, out_ch, (1, 1), (1, 1), (0, 0))
        self.conv2 = nn.Conv2d(out_ch, out_ch, (1, 1), (1, 1), (0, 0))

    def forward(self, vertex, light_img_feature):
        ...
        x4_cat = torch.cat((v4, light_img_feature), 1)
        x4 = self.merge(x4_cat)

        # Upsample 1
        x3 = F.interpolate(x4, v3.size()[2:], mode='nearest')  # 128,128
        x3 = self.uconv1(torch.cat((x3, v3), 1))

        # Upsample 2
        x2 = F.interpolate(x3, v2.size()[2:], mode='nearest')  # 128,128
        x2 = self.uconv2(torch.cat((x2, v2), 1))

        # Upsample 3
        x1 = F.interpolate(x2, v1.size()[2:], mode='nearest')  # 128,128
        x1 = self.uconv3(torch.cat((x1, v1), 1))
