...
Based on the implementation mentioned above, a Gated Convolutional Neural Network based on UNet proposed by \cite{unet} is proposed to perform normal inference. The architecture of network is shown in Figure \ref{fig:gcnn-archi}. It takes 3D vertex with size $ 512\times512\times3 $ as input, then samples downward 3 times, each scale with 3 gated convolution layers. The upsampling part interpolate the feature maps 3 times with 1 gated convolution layer in each scale. It keeps the skip connection in UNet to remain the fine detail features. It the end, two standard convolutional layers have been added, the output normal map has the same size as the input 3D vertex map.
