...
Before we introduce this approach, let us discuss what we can actually get from the images.
...
where $ \oplus $ denotes the element-wise product. This decomposition of the image is based on the intrinsic image model, which was proposed by  \cite{intrinsic-image}. It interprets the observed image into a reflectance image and a shading image. Figure \ref{fig:intrinsic-image} gives a visualization of the decomposition.
...
\begin{figure}[th]
	...
	\decoRule
	\caption{Intrinsic image analysis of the Washington object. From left to right, original image, reflectance image, shading image, light image, normal image.}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\node (conv-oplus) [below of=conv-output, yshift=-.2cm] {$\Huge\oplus $};
		\node (conv-LeakyReLU) [rect,below of=conv-oplus, yshift=-0.4cm] {Feature};
		\node (conv-conv2) [rect,below of=conv-LeakyReLU, yshift=-1cm] {Feature};
		\node (conv-input) [below of=conv-oplus] at (6,-7) {\includegraphics[width=.25\textwidth]{./Figures/train-input.png}};
		
		\draw [arrow] (conv-oplus) -- (conv-output);
		\draw [arrow] (conv-LeakyReLU) -- (conv-oplus);
...
In this pipe, we take four kinds of feature maps $ \textbf{X}_{L4D}, \textbf{X}_{L3U},\textbf{X}_{L2U}, \textbf{X}_{L1U} $ as the guided information for the further operation.
...
In the end of the network, in order to fit the normal inference task, two standard convolution layers are added to output normal map $ \textbf{X}_{N} \in  \mathbb{R}^{\frac{W}{8}\times \frac{H}{8}\times 3} $ (represented as a regression function $ n $)
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\begin{axis} [axis lines=center]
			\addplot [domain=0:1, smooth, thick, width=5mm, color=red] { x };
			\addplot [domain=1:2, smooth, width=5mm] { x };
			\addplot [domain=-2:-1, smooth, width=3mm] { -x };
			\addplot [domain=-1:0, smooth, thick, width=3mm, color=red] { -x };
			\addplot [domain=-2:-1, smooth, thick, color=red] { (x^2+1^2)/(2*1) };
			\addplot [domain=1:2, smooth, thick, color=red] { (x^2+1^2)/(2*1) };
		\end{axis}
	\end{tikzpicture}
