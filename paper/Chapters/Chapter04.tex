...
First we need geometry information of the object surface. This can be collected by a depth camera, which records the Z-axis distance of the object surface to the camera position and it can be further converted to a point cloud. 
Second we need illuminated information of the object surface, which considers as a photometric-information of for the further accuracy improvement. 
This kind of information requires an observed image of the object with the light directions that projected to the object surface. 
...
Besides, some of the objects also has colored texture, as shown in the second row of Figure \ref{fig:dataset-demo}. This is specially prepared for illuminated approach since they will take the image as account to evaluate the surface normal. The textured models are more likely to the real dataset and further gives a good robustness to our models.
...
In order to simulate the data collection scenario close to the real life in the laboratory as much as possible. We did following settings. A flat cylinder called $ stage $ is placed in the center in the 3D space as a platform to place objects. The stage is fixed in a determined position, which is not moved when the images are captured. 
A directional light with RGB color FFF4D6 \col{direction-light-color} is placed in a top-view direction around 25 meters away to provide as ambient light source, which also has a fixed position.
A RGB-D camera is placed in a top-view direction around 10m away from the stage. The camera is the sensor to capture the depth map and the grayscale image, which is random change the places after each scene. The moving range of the camera has 0.1m in both directions of X, Y, and Z axis and 1 degree random changed Euler angle. 
A point light placed around 5 meters away as the illumination light, which is also randomly moved after each scene. The moving range is 0.3m in both directions of X, Y, and Z axis and 1 degree random in Euler angle.
