...
Another model called ``NOC" is designed to verify the skip connection, which simply removes the skip connections in the network but keeps other settings same. Is is designed to show to which content will skip connection help the model performance. 

We use Reversed Huber Loss as loss function during the training, since we found it gives a better final error compare to L2 loss. The model is evaluated after every epoch training and calculate the angle error on the test dataset. The training history is shown in Figure \ref{fig:gcnn-training-history}. We can see that all of these three models can learn the normal inference task where the model ``NOC" has the highest error, which is the effect of the missing of skip connection. The ``CNN" model has the skip connection but use standard convolution layer has a better performance. The GCNN model use the skip connection and the gated convolution layer further reduce the error.
