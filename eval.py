...
def main():
    ...
    for idx in eval_list:
        data, depth, depth_noise, normal_gt, img = file_io.load_single_data(path, idx=idx)
        ...
        # svd normal
        normal_svd, svd_img = svd.eval(vertex, farthest_neighbour=2)
        ...
        if path == config.real_data:
            ...
            normal_noraml, normal_img, _, _ = eval.eval(vertex, normal_model_path, k=2, output_type='normal')
        ...
        nnnn_noraml, nnnn_img, _, _ = eval.eval(vertex, normal_model_path, k=1, output_type='normal')
