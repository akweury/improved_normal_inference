...
def resize_matrix(matrix, resize_factor):
    iter_times = resize_factor
    resized_matrix = matrix.copy()
    for i in range(iter_times):
        resized_matrix = resized_matrix[::2, 1::2]
...
def convert2training_tensor_resized(path, k, output_type='normal', resize_factor=2):
    ...
    for item in range(len(data_files)):
        ...
        gt_normal_resize = resize_matrix(gt_normal, resize_factor)
        light_direction_gt_resize = resize_matrix(light_direction_gt, resize_factor)
        img_resize = resize_matrix(img, resize_factor)
        vertex_norm_resize = resize_matrix(vertex_norm, resize_factor)
        light_direction_resize = resize_matrix(light_direction, resize_factor)

        ###################  target  ####################
        target = np.c_[
            gt_normal_resize,  # 0,1,2
            np.sum(gt_normal_resize * light_direction_gt_resize, axis=-1, keepdims=True),  # 3
            np.expand_dims(img_resize, axis=2),  # 4
            light_direction_gt_resize  # 5,6,7
        ]

        ################### input #######################
        vertex_norm[mask] = 0
        vectors = np.c_[vertex_norm_resize,  # 0,1,2
                        np.expand_dims(img_resize, axis=2),  # 3
                        light_direction_resize]  # 4,5,6
...
if args.data in ["synthetic128", "synthetic256", "synthetic512", "synthetic64"]:
    for folder in ["baoshanlu", "bus", "dragon", "garfield", "washington"]:
        ...
        if args.machine == "remote":
            ...
            original_folder = config.synthetic_data / args.data / "separate" / folder
            dataset_folder = config.synthetic_data_noise_local / args.data / "separate" / folder
        else:
            raise ValueError
    ...
    for folder in ["test", "train"]:
        ...
        for k in args.max_k.split(','):
            print(f"K = {k}, {dataset_folder}")
            convert2training_tensor_resized(dataset_folder, k=int(k), output_type="normal_noise")
else:
    raise ValueError
