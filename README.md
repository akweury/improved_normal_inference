...
CUDA_VISIBLE_DEVICES=2 python3 main.py --machine remote --exp albedoGated --dataset synthetic512 --batch_size 8 --train-on 50 
    ...
    srun \
      --job-name="INI-an2-f4-8-49-huber" \
      --time=3-00:00 \
      -p A100 \
      --ntasks=1 \
      --gpus-per-task=1 \
      ...
      --container-workdir="`pwd`" \
      --container-mounts=/netscratch/$USER:/netscratch/$USER,/ds:/ds:ro,"`pwd`":"`pwd`" \
      python3 main.py --machine remote --exp an2 --dataset synthetic128 --batch_size 8 --lightNumUse 1 --lr-scheduler 8,49 --print-freq 1 --net_type gnet-f4
