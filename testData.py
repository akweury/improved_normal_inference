    ...
    for i in range(vertex.shape[0]):
        for j in range(vertex.shape[1]):
            if mask[i,j] == True:
                ...
                file.write(str(int(image[i,j,0])) + ' ' + str(int(image[i,j,0])) + ' ' + str(int(image[i,j,0]))+'\n')      
    if cameraPoints != None:
...
vertex = depth2vertex(torch.tensor(depth).permute(2, 0, 1), torch.tensor(data['K']), torch.tensor(data['R']), torch.tensor(data['t']))
...
for i in range(len(cameraPoints)):
    cameraPoints[i] = np.array(data['R']).transpose() @ cameraPoints[i]/4 - np.array(data['R']).transpose() @ np.array(data['t'])
